...
export function getOverallSemaphore(result, totals, validationReport) {
  const validationIssues = result?.validation?.issues || [];
  const hasBlockingFormatErrors = validationIssues.some((issue) => issue.level === "error");
  const hasWarnings = validationIssues.some((issue) => issue.level === "warning");
  const hasWritingErrors = validationReport?.videos?.some((video) => !video.passed) || false;
  const needsFinalReview = !validationReport;

  if (hasBlockingFormatErrors || hasWritingErrors) {
    return {
      tone: "danger",
      label: "Rojo",
  ...
  if (!totals?.inTargetRange || hasWarnings || needsFinalReview) {
    return {
      tone: "warning",
      label: "Amarillo",
...
export function getNextActionCard({ result, validationReport, totals, recommendedVideo }) {
  ...
  if (result?.validation?.issues?.some((issue) => issue.level === "error")) {
    return {
      tone: "danger",
